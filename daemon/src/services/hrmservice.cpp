...
void HRMService::enableRealtimeHRMeasurement(bool enable)
{
    ...
    if (enable) {
        writeValue(UUID_CHARACTERISTIC_HRM_CONTROL, UCHAR_TO_BYTEARRAY(COMMAND_SET_HR_MANUAL_DISABLE));
        writeValue(UUID_CHARACTERISTIC_HRM_CONTROL, UCHAR_TO_BYTEARRAY(COMMAND_SET_HR_CONTINUOUS_ENABLE));
    } else {
        writeValue(UUID_CHARACTERISTIC_HRM_CONTROL, UCHAR_TO_BYTEARRAY(COMMAND_SET_HR_CONTINUOUS_DISABLE));
...
void HRMService::enableManualHRMeasurement(bool enable)
{
    ...
    if (enable) {
        writeValue(UUID_CHARACTERISTIC_HRM_CONTROL, UCHAR_TO_BYTEARRAY(COMMAND_SET_HR_CONTINUOUS_DISABLE));
        writeValue(UUID_CHARACTERISTIC_HRM_CONTROL, UCHAR_TO_BYTEARRAY(COMMAND_SET_HR_MANUAL_ENABLE));
    } else {
        writeValue(UUID_CHARACTERISTIC_HRM_CONTROL, UCHAR_TO_BYTEARRAY(COMMAND_SET_HR_MANUAL_DISABLE));
        disableNotification(UUID_CHARACTERISTIC_HRM_CONTROL);
    }
...
void HRMService::setHeartrateSleepSupport()
{
    ...
    QByteArray cmd;
    if (enable) {
        cmd = UCHAR_TO_BYTEARRAY(COMMAND_ENABLE_HR_SLEEP_MEASUREMENT);
    } else {
        cmd = UCHAR_TO_BYTEARRAY(COMMAND_DISABLE_HR_SLEEP_MEASUREMENT);
    }
...
{
    writeAsync(UUID_CHARACTERISTIC_HRM_CONTROL, UCHAR_TO_BYTEARRAY(COMMAND_SET_HR_CONTINUOUS_ENABLE));
}

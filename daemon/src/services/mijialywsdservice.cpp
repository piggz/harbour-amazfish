...
MijiaLywsdService::MijiaLywsdService(const QString& path, QObject* parent)
    : QBLEService(UUID_SERVICE_MIJIA_LYWSD, path, parent)
{
    qDebug() << Q_FUNC_INFO;

    connect(this, &QBLEService::characteristicRead, this,
        &MijiaLywsdService::characteristicRead);
    connect(this, &QBLEService::characteristicChanged, this,
        &MijiaLywsdService::characteristicChanged);
}
...
void MijiaLywsdService::characteristicChanged(const QString& characteristic,
    const QByteArray& value)
{
    characteristicRead(characteristic, value);
}

void MijiaLywsdService::characteristicRead(const QString& characteristic,
    const QByteArray& value)
{
    ...
    if (characteristic == UUID_CHARACTERISTIC_LIVE_DATA) {
        if (value.length() == 5) {
            ...
            qDebug() << "LYWSD03MMC Live Data - Temperature:" << temperature
                     << "Humidity:" << humidity << "Voltage:" << voltage;

            emit informationChanged(Amazfish::Info::INFO_TEMPERATURE,
                QString::number(temperature, 'f', 2));
            emit informationChanged(Amazfish::Info::INFO_HUMIDITY,
                QString::number(humidity));
            emit informationChanged(Amazfish::Info::INFO_VOLTAGE,

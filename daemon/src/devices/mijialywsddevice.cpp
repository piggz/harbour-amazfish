    ...
    : AbstractDevice(pairedName, parent)
{
    ...
    connect(this, &QBLEDevice::propertiesChanged, this,
        &MijiaLywsdDevice::onPropertiesChanged, Qt::UniqueConnection);
}
...
QString MijiaLywsdDevice::deviceType() const { return "Xiaomi Thermometer"; }

AbstractFirmwareInfo* MijiaLywsdDevice::firmwareInfo(const QByteArray& bytes,
...
{
    qDebug() << Q_FUNC_INFO;
    DeviceInfoService* info = qobject_cast<DeviceInfoService*>(
        service(DeviceInfoService::UUID_SERVICE_DEVICEINFO));
    if (info) {
        info->refreshInformation();
    }

    MijiaLywsdService* lywsd = qobject_cast<MijiaLywsdService*>(
        service(MijiaLywsdService::UUID_SERVICE_MIJIA_LYWSD));
    if (lywsd) {
        lywsd->refreshInformation();
    }
}

void MijiaLywsdDevice::sendAlert(
    const Amazfish::WatchNotification& notification)
{
    Q_UNUSED(notification)
}

void MijiaLywsdDevice::incomingCall(const QString& caller) { Q_UNUSED(caller) }

void MijiaLywsdDevice::incomingCallEnded() { }

QBLEService* MijiaLywsdDevice::drv_createService(const QString& uuid,
    const QString& path)
{
...
void MijiaLywsdDevice::onPropertiesChanged(QString interface, QVariantMap map,
    QStringList list)
{
...
void MijiaLywsdDevice::initialise()
{
    ...
    MijiaLywsdService* lywsd = qobject_cast<MijiaLywsdService*>(
        service(MijiaLywsdService::UUID_SERVICE_MIJIA_LYWSD));
    if (lywsd) {
        connect(lywsd, &MijiaLywsdService::informationChanged, this,
            &AbstractDevice::informationChanged, Qt::UniqueConnection);
    ...
    DeviceInfoService* info = qobject_cast<DeviceInfoService*>(
        service(DeviceInfoService::UUID_SERVICE_DEVICEINFO));
    if (info) {
        connect(info, &DeviceInfoService::informationChanged, this,
            &AbstractDevice::informationChanged, Qt::UniqueConnection);
    }

        ...
        if (curs) {
            if (curs->open() && curs->moveFirst()) {
                while (!curs->eof()) {
                    ...
                    int numStages = TypeConversion::toInt32(raw, offset);
                    ...
                    ss.lightSleepDuration = totalLightMinutes;
                    ss.deepSleepDuration = totalDeepMinutes;
                    ss.remSleepDuration = totalRemMinutes;
                    ss.awakeDuration = totalWakeMinutes;
                    sessions << ss;
